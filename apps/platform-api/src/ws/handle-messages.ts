          ...
          */
            ...
            try {
              await db.transaction(async (tx) => {
                ...
                const market = await db.select().from(marketTable).where(eq(marketTable.marketId, pending.marketId))
                ...
                pending.res.status(200).json({success: true, message: "Order has been placed"})
              })
            } catch (error) {
              console.error(error)
              pending.res.status(errorStatusCode!).json({success: false, message: "Unable to perform transactions, Internal server error", reason: errosMessage})
            }
        }
          ...
          */
            ...
            try {
              await db.transaction(async (tx) => {
                ...
                const userAccount = await db.select().from(usersTable).where(eq(usersTable.userId, pending.userId));
                const market = await db.select().from(marketTable).where(eq(marketTable.marketId, pending.marketId))

                const returnBalance = data.returnToUser;
                const newBalance = Number(userAccount[0].userWalletBalance) + returnBalance;
                ...
                await tx.insert(orderTable).values({
                  ...
                  orderType: pending.orderType,
                  qty: pending.userOrderQty,
                  executionPrice: (data.returnToUser / pending.userOrderQty).toString(),
                  orderPlacedBy: pending.userId,
                  marketId: pending.marketId
                })
              ...
              console.error(error)
              pending.res.status(500).json({success: false, message: "Unable to perform transactions. Internal server error"})
            }
        }
