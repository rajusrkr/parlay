  ...
  try {
    ...
    const markets = await db
      .select({
        ...
        thumbnailImage: marketTable.marketThumbnailImageUrl,
      ...
      .from(marketTable)
      .where(eq(marketTable.currentStatus, "OPEN"));
    ...
    const prices = await db
      .select()
      .from(priceData)
      .orderBy(asc(priceData.priceUpdatedOn));

    if (prices.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Prices are not available" });
    }
    ...
    const pricesMap = new Map<
      string,
      Array<{
        yes: { value: number; time: number };
        no: { value: number; time: number };
      }>
    >();
    ...
    for (const price of prices) {
      ...
      pricesMap
        ...
        .push({
          yes: {
            time: price.priceUpdatedOn!,
            value: Number(price.yesSidePrice),
          },
          no: { time: price.priceUpdatedOn!, value: Number(price.noSidePrice) },
        });
    }

    // merge those two, markets and price map
